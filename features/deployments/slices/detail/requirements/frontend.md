...
| Действия | кнопки по правам/статусу бэкенда | скрыты/недоступны, если нет прав; для методолога только редактирование артефактов |

## Статусы и действия

- UI показывает статус из бэкенда без локальной подмены.
- `NEW` — доступны действия по правам.
- `ON_APPROVAL` — показываем действия, которые вернул бэкенд; если пользователь редактирует и сохраняет, карточка возвращается в `NEW`; без `recall/startRatification` из старой модели.
