...
| Действия | показываем только то, что разрешено бэкендом/ролью; не добавляем фронтовые переходы |
...
| карточка `ON_APPROVAL` | карточка на согласовании | действия по `availableActions`; сохранение после `edit` возвращает в `NEW` |
