...
Зафиксировать общий бэкенд-контракт рабочей области: один источник статуса, один набор действий, создание без черновика-оболочки и read-only связь со SberDocs-согласованием.
...
| Действия | `submitForApproval`, `edit`, `deploy`, `toArchive`; решения согласования выполняются в SberDocs |
| `ON_APPROVAL` | backend отдаёт read-only status/link/snapshot SberDocs, если доступно, и не возвращает локальные `approve`/`reject` |
...
| Approval/SberDocs integration | бриф, отправка, статус согласования, история, комментарии и решения |
...
- [ ] Бэкенд не возвращает локальные `approve`/`reject` как действия Deployments в SberDocs-сценарии.
