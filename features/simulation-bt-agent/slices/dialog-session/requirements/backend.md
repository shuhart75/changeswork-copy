...
Источником правды для run status, terminal status, истории и агентского контекста является RAIN по принятому `agent_openapi_1.yaml`, доступный АС КОДА через REST-фасад RAIN. Backend АС КОДА является integration boundary: авторизует пользователя, хранит UI-сессию и внутренний `agent_dialog_run_ref` между `session_id` и `run_id` RAIN, вызывает фасад RAIN server-to-server, нормализует ответы для frontend и может вести локальную копию/кэш истории для UI и аудита.
...
- `REST-фасад RAIN` — внешний server-to-server API, с которым интегрируется АС КОДА; фасад принимает run, возвращает HTTP/API ошибки фасада и отдаёт статусы внутреннего агента через status/result endpoints.
...
- `history window` — набор страниц истории, загруженных frontend в текущем открытии панели; backend не хранит это состояние и не вычисляет для него отдельные признаки.
...
- `GET /dialog/messages` возвращает `older_cursor` для движения в прошлое;
- status endpoint не знает, что уже видел frontend, и не возвращает `history_changed`.

### BR-5. `history_changed` не используется
...
- `GET /dialog/status` возвращает только состояние обработки сессии, а не состояние истории;
- после terminal status frontend запрашивает latest page истории через `GET /dialog/messages?session_id=...&limit=20`;
- если пользователь не находится внизу истории, frontend сам показывает индикатор нового сообщения после получения latest page и не прокручивает чат автоматически.
...
### BR-7. Ошибки фасада и ошибки агента обрабатываются отдельно
- ошибки фасада RAIN приходят как HTTP/API error response на server-to-server вызов до принятия run, например `400`, `409`, `5xx`, transport/read timeout или неготовность фасада;
- ошибки агента не приходят как HTTP error response создания run: они возвращаются фасадом в status/result модели как terminal `failed` или `timeout`;
- если фасад вернул `202 Accepted`, run считается принятым, повторный `POST /chat/runs` больше не выполняется, а backend только poll-ит фасад через `GET /chat/runs/{run_id}`;
- retry допустим только для retryable ошибок фасада/transport до получения `202 Accepted`; validation `400`, conflict активного run и terminal `failed`/`timeout` не retry-ятся;
- agent timeout может быть получен только через status фасада; backend нормализует его в `dialog_status=timeout`, а frontend должен предложить пользователю перезапуск сессии с новым `session_id`.

### BR-8. SLA RAIN учитывается через timeout policy
...
- timeout frontend/backend polling не означает timeout RAIN run; terminal timeout должен приходить только из статуса, который вернул фасад RAIN.
...
- уточнение перечня retryable ошибок фасада и параметров инфраструктурных timeout;
...
### Сценарий BE-3. Timeout или ошибка агента
1. Frontend вызывает `GET /dialog/status?session_id=...`.
2. Backend вызывает RAIN `GET /chat/runs/{run_id}`.
3. Фасад RAIN возвращает terminal `failed` или `timeout` внутреннего агента.
4. Backend нормализует ошибку для frontend, не раскрывая внутренние детали RAIN.
5. Backend не повторяет создание run; при `timeout` frontend получает рекомендацию начать новую сессию с новым `session_id`.
...
4. Backend возвращает `session_id`, `dialog_status`, `can_send_message` и ошибку.
...
- ответ содержит `session_id`, `can_send_message=false`, `dialog_status=queued`; `history_changed` не возвращается.
...
**Описание:**
Backend должен вызывать RAIN строго по принятому `agent_openapi_1.yaml`. Полная версия RAIN OpenAPI также включена в общие требования `features/simulation-bt-agent/requirements.md` в разделе `Полный OpenAPI-контракт RAIN`; этот slice не должен заводить альтернативный RAIN-контракт.
...
**Правила и ограничения:**
- endpoint `GET /dialog/status?session_id=...` возвращает `session_id`, `can_send_message`, `dialog_status` и optional `error`;
...
- frontend берёт машинный код из `DialogSessionView.error.code`; backend заполняет его из безопасной нормализации `RAIN RunStatusResponse.error.code` или из собственного кода ошибки АС КОДА;
...
- endpoint не возвращает `history_changed`; frontend после terminal `succeeded` сам запрашивает latest page истории.
...
- backend возвращает `older_cursor` и `has_more_before`; технические счётчики RAIN не требуются во frontend-facing MVP API;
...
**Описание:**
Backend должен иметь явную политику timeout/retry для вызовов REST-фасада RAIN и не смешивать её с terminal timeout агента.
...
- retry `POST /chat/runs` допускается только при retryable ошибке фасада/transport до получения `202 Accepted` и только с тем же `Idempotency-Key`;
- после `202 Accepted` backend не повторяет `POST /chat/runs`, даже если последующий polling длится долго;
- если `GET /chat/runs/{run_id}` вернул terminal `timeout`, backend нормализует этот статус для frontend как timeout агента;
- сетевой timeout polling-запроса АС КОДА к фасаду RAIN не означает terminal `timeout` run и обрабатывается как временная ошибка чтения статуса;
- для технических ошибок до принятия run фасадом допускается retry по инфраструктурной политике, но он должен быть прозрачно логирован;
- terminal `failed`/`timeout` агента не retry-ятся автоматически; при `timeout` backend возвращает frontend ошибку, после которой доступный путь пользователя — перезапуск сессии с новым `session_id`.
...
| `GET /dialog/status?session_id=...` | получить статус сессии | frontend polling | `run_id` и `history_changed` не раскрываются frontend |
        ...
        "200":
          ...
          content:
            application/json:
              schema:
                type: object
                required: [items, older_cursor, has_more_before]
                properties:
                  items:
                    type: array
                    items:
...
- Вычисление статусов / derived fields: `dialog_status`, `can_send_message`, `has_more_before`;
- Идемпотентность / ретраи: retry `POST /chat/runs` возможен только для retryable ошибки фасада/transport до `202 Accepted` и только с тем же `Idempotency-Key`; после `202 Accepted` выполняется только polling фасада.
...
Backend возвращает frontend-facing ошибки в формате `{"error":{"code":"...","message":"...","retryable":...}}`. Для terminal `failed`/`timeout` тот же объект передаётся как `DialogSessionView.error`; frontend берёт код из `DialogSessionView.error.code`.

| HTTP status | `error.code` для frontend | Источник кода | Условие | Ответ |
|---|---|---|---|---|
| `400` | `message_empty` | backend validation `POST /dialog/message`; frontend может выявить тот же код локально до запроса | пустой текст | ошибка валидации |
| `400` | `message_too_long` | backend validation `POST /dialog/message`; frontend может выявить тот же код локально до запроса | prompt превышает лимит `3000` | ошибка валидации с текущим лимитом |
| `404` | `session_not_found` | backend ownership/session validation | `session_id` не найден или не принадлежит пользователю | ошибка отсутствующей сессии |
| `409` | `run_in_progress` | backend active run lock | уже есть active run | конфликт |
| `503` | `agent_not_ready` | backend readiness normalization from RAIN `/health/readiness` | RAIN readiness не готов | временная недоступность |
| `504` | `rain_status_unavailable` | backend polling/read timeout while reading facade `GET /chat/runs/{run_id}` | не удалось прочитать статус run через фасад | временная ошибка чтения статуса, не terminal status run |
| `200` | `agent_error` | `DialogSessionView.error.code`, нормализованный из terminal status/error фасада | агент вернул terminal `failed` | terminal status `failed`, без retry создания run |
...
- Что должно попасть в release finalization: frontend session-level contract, RAIN facade async contract, лимиты, timeout/retry policy, health/status behavior и правила проксирования истории.

## Observability и аудит

- Логи: создание RAIN run, чтение статуса RAIN, terminal status, timeout чтения статуса, `error.code`, `session_id`, `run_id`;
...
- [ ] Retry выполняется только для retryable ошибок фасада до `202 Accepted`
- [ ] После `202 Accepted` backend не повторяет создание run и только poll-ит фасад
- [ ] При terminal `timeout` пользователь получает путь перезапуска сессии с новым `session_id`
