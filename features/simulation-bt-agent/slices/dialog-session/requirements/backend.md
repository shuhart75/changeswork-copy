...
| `504` | `rain_status_unavailable` | backend polling/read timeout while reading RAIN `GET /chat/runs/{run_id}` | не удалось прочитать статус RAIN run | временная ошибка чтения статуса, не terminal status run |
| `200` | `agent_error` | `DialogSessionView.error.code`, нормализованный из `RAIN RunStatusResponse.error.code` или некорректного ответа RAIN | RAIN terminal `failed` | terminal status `failed` |
| `200` | `generation_timeout` или `agent_timeout` | `DialogSessionView.error.code`, нормализованный из terminal status/error RAIN | RAIN terminal `timeout` | terminal status `timeout` |
