...
RAIN по принятому контракту предоставляет async REST API через REST-фасад: `POST /chat/runs`, `GET /chat/runs/{run_id}` и постраничную историю. Frontend не вызывает RAIN напрямую: он работает с backend АС КОДА, получает backend-managed `session_id`, подтверждение принятия сообщения и polling-ом читает session-level view без `run_id`.
...
- понятное сообщение об ошибке/timeout; при agent timeout пользователь получает рекомендацию перезапустить сессию с новым `session_id`.
...
### Сценарий FE-3. Timeout или ошибка агента
1. Backend через polling фасада получает terminal `failed` или `timeout` и возвращает его как `dialog_status`.
2. Frontend показывает понятное сообщение об ошибке.
3. Frontend не повторяет отправку автоматически.
4. При `timeout` frontend предлагает перезапустить сессию, то есть начать заново без старого `session_id`.
...
- при `failed` frontend показывает ошибку агента без автоматического retry отправки;
- при `timeout` frontend показывает сообщение о timeout агента и предлагает перезапустить сессию с новым `session_id`;
...
- при polling timeout/network error frontend может повторить чтение статуса по согласованной retry policy, потому что это ошибка чтения фасада, а не terminal timeout агента;
- если `GET /dialog/status?session_id=...` вернул terminal `timeout`, frontend не должен повторять `POST /dialog/message` или `POST /chat/runs` через backend.
...
| Не удалось отправить сообщение | `rain_facade_unavailable`, `rain_status_unavailable` или transport error | `response.body.error.code` от `POST /dialog/message`; при network error frontend ставит локальный код transport-сбоя | `Не удалось отправить сообщение агенту.` | внутри окна |
| Run завершился timeout | `generation_timeout` или `agent_timeout` | `DialogSessionView.error.code` из `GET /dialog/status?session_id=...`, нормализованный backend из terminal status/error фасада | `Агент долго не отвечает. Перезапустите сессию агента.` | внутри окна |
| Run завершился ошибкой | `agent_error` | `DialogSessionView.error.code` из `GET /dialog/status?session_id=...`, нормализованный backend из terminal status/error фасада | `Не удалось получить результат от агента. Попробуйте позже.` | внутри окна |
...
- [ ] При terminal `timeout` frontend предлагает новый `session_id`, а не автоматический retry
