...
- `polling` — периодический запрос frontend к backend АС КОДА для чтения `dialog_status`, `can_send_message` и optional признака `history_changed`.
- `cursor` — маркер для порционной загрузки более ранних сообщений истории.
- `sync_cursor` — opaque marker последней синхронизации актуального хвоста истории; frontend получает его из `GET /dialog/messages` и передаёт как `known_sync_cursor` в `GET /dialog/status`, если хочет получить `history_changed`.
...
2. Frontend polling-ом вызывает `GET /dialog/status?session_id=...&known_sync_cursor=...`, где `known_sync_cursor` взят из последнего ответа `GET /dialog/messages`.
3. Backend возвращает `history_changed=true`, если в RAIN появились сообщения после этого cursor.
4. Frontend не меняет текущий scroll-position и не переносит пользователя вниз автоматически.
5. UI показывает sticky-индикатор `Новое сообщение` или счётчик новых сообщений у нижней границы истории.
6. По клику на индикатор frontend загружает последние сообщения, например `GET /dialog/messages?session_id=...&limit=20`, сохраняет новый `sync_cursor` и прокручивает историю вниз.
...
- backend возвращает `202 Accepted` с `session_id`, `can_send_message=false` и `dialog_status`; `history_changed` не требуется до первой синхронизации истории;
...
- frontend вызывает `GET /dialog/status?session_id=...&known_sync_cursor=...`, если у него есть `sync_cursor` последней синхронизации истории;
- status response содержит `session_id`, `can_send_message`, `dialog_status`, optional `history_changed` и optional `error`; `history_changed` отсутствует или `null`, если frontend не передал `known_sync_cursor` или backend не смог выполнить проверку истории;
- polling завершается, когда `dialog_status` находится в terminal status: `succeeded`, `failed`, `timeout`, `cancelled`;
- при `succeeded` frontend синхронизирует историю даже если `history_changed` не пришёл;
...
**Детали:**
- при открытии окна загружается последняя страница истории, рекомендуемо `limit=20`, и frontend сохраняет полученный `sync_cursor`;
...
- после terminal status frontend перечитывает последние `20` сообщений или использует `history_changed`, вычисленный через `known_sync_cursor`;
...
| Индикатор нового сообщения | `history_changed` из session status при переданном `known_sync_cursor` | отсутствует | pending update | ошибка синхронизации | показывается, если пользователь не внизу истории |
...
| `GET /dialog/status?session_id=...&known_sync_cursor=...` | polling статуса сессии | читаем `session_id`, `can_send_message`, `dialog_status`, optional `history_changed`, ошибку | пока `dialog_status` активен или нужна синхронизация | `known_sync_cursor` передаётся только после загрузки истории; отдельный status по `run_id` не нужен |
| `GET /dialog/messages?session_id=...&limit=&before=` | загрузка истории | читаем страницу сообщений, `older_cursor`, `sync_cursor`, `has_more_before` | при открытии окна и догрузке истории | не загружает всю историю сразу; `sync_cursor` хранится как opaque marker |
...
- нельзя ожидать, что backend помнит загруженную frontend историю: для `history_changed` frontend должен передать `known_sync_cursor`;
