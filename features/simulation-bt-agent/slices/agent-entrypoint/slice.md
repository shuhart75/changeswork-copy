# Slice — Точка входа, UI-сессия и статус агента
...
## Назначение
Зафиксировать интерфейсный и системный контракт для открытия окна агента из интерфейса симуляций, управления UI-сессией АС КОДА по `session_id`, отображения статуса агента по `liveness/readiness` и использования уже реализованной деталки симуляции как источника признаков доступности БТ-сценария.
