...
## Назначение
Зафиксировать интерфейсный и системный контракт для открытия окна агента из интерфейса симуляций, получения backend-managed `session_id`, отображения статуса агента по `liveness/readiness` и использования уже реализованной деталки симуляции как источника признаков доступности БТ-сценария.
