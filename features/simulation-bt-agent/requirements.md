...
| BT-сценарий недоступен для симуляции | `bt_context_not_available` | frontend context validation или `response.body.error.code` от `POST /dialog/message` | `Сформировать БТ можно только для завершённой симуляции, доступной к выводу в ПРОМ без табличных риск-параметров и изменений только для "Действует с" (дата).` |
...
| Невалидные риск-параметры | `bt_risk_params_invalid` | `response.body.error.code` от `POST /dialog/message` | `Изменённые риск-параметры не соответствуют условиям формирования БТ.` |
...
- Бизнес-правила: action `Сформировать БТ для текущей симуляции` доступен только для конкретной успешно завершённой симуляции с опцией вывода в ПРОМ, без табличных риск-параметров, без изменений только верхней границы скалярных риск-параметров и без изменений только поля "Действует с" (дата); клик по action только вставляет черновик запроса; пользователь может редактировать текст или не отправлять его; публикация БТ выполняется только после явного подтверждения внутри диалога с агентом; после успешной публикации агент возвращает ответ, в котором должна быть ссылка на БТ; автоматическое сохранение ссылки на БТ в данных симуляции в MVP не выполняется.
- Ограничения: черновик запроса формируется на стороне АС КОДА по данным existing simulation detail API; промпт включает ФИО пользователя и блок `Поменяй значение риск-параметров:` только для скалярных риск-параметров с изменённой "Нижней границей"; табличные параметры, изменения только "Верхней границы" и изменения только "Действует с" не включаются; RAIN `POST /chat/runs` принимает optional `risk_params` и `simulation_id`; результат создания БТ должен читаться через `GET /chat/runs/{run_id}/result` и структурированные `artifacts[]`.
...
3. При отправке БТ-контекста backend проверяет отсутствие табличных риск-параметров, изменений только "Верхней границы" и изменений только "Действует с" (дата), формирует промпт по нижним границам и передаёт RAIN `simulation_id` и `risk_params`, если данные доступны и валидны.
...
- показать inline action БТ только в подходящем контексте и при готовом агенте; если frontend видит в данных страницы табличные риск-параметры, изменения только "Верхней границы" или изменения только "Действует с" (дата), action скрывается или блокируется с причиной;
- по клику вставлять в поле ввода черновик запроса по шаблону, используя ФИО пользователя и данные уже реализованного `GET /api/v1/simulation/{number}`; блок `Поменяй значение риск-параметров:` включает только скалярные параметры с изменённой "Нижней границей";
...
- перед вызовом RAIN проверять, что пользователь имеет доступ к указанной симуляции и что она подходит для BT-сценария: завершена, доступна к выводу в ПРОМ, не содержит изменённых табличных риск-параметров, изменений только "Верхней границы" и изменений только "Действует с" (дата);
- формировать `message` для БТ по шаблону `Меня зовут %username%.` + optional блок `Поменяй значение риск-параметров:` со скалярными параметрами, у которых изменилась "Нижняя граница";
