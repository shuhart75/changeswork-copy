# Slice — Форма симуляции

Статус: **импортировано как existing baseline coverage**  
Feature: `features/simulations/feature.md`  
Порядок в feature requirements: `03`  
Дата обновления: `2026-04-24`

## Назначение
Зафиксировать существующую форму создания/редактирования симуляции с полями, валидацией и сохранением в статусе `draft`.

## Связанные planning stories
- Для этой imported existing feature planning stories не ведутся.

## Источники
- `../../references.md`
- `context/source-materials/legacy/changeswork-full/planning/mvp/current/tasks/legacy/mvp_tasks_simulation_form.md`

## Пакеты требований
- `../../requirements.md`
- `requirements/frontend.md`
- `requirements/backend.md`

## Связанные прототипы
- legacy simulation form макет отслеживается только в raw snapshot

## Связанные execution-артефакты
- `execution/tasks.md`
