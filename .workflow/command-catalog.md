...
Нужно подготовить `features/<feature>/requirements.md` по шаблону, зафиксировать там порядок и границы slice sections, а потом разложить этот root document по detail packs, обновить domain-impact и перечислить затронутые прототипы.
```
...
| `делаем требования` | `requirements` | Switch mode, read baseline/current and author the root feature requirements before deriving slices. |
