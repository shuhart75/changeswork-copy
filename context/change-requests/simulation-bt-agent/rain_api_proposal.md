...
Отдельный endpoint создания сессии не требуется. Сессия создаётся или восстанавливается лениво:

- АС КОДА генерирует `session_id` на своей стороне при открытии UI-сессии агента;
- RAIN получает `session_id` в `POST /chat/runs`;
- если такой `session_id` уже существует у RAIN, RAIN добавляет новый `run` и сообщения в существующую историю;
- если такого `session_id` ещё нет, RAIN создаёт сессию автоматически в рамках обработки первого `POST /chat/runs`;
- `GET /chat/sessions/{session_id}/messages` для неизвестной сессии возвращает `404 session_not_found`, если по этой сессии ещё не было ни одного `run`.

Так мы не вводим отдельный подготовительный вызов `PUT /chat/sessions/{session_id}`. Для пользователя открытие панели агента остаётся быстрым и не зависит от RAIN; первый server-to-server контакт с RAIN происходит только при отправке сообщения или при чтении уже существующей истории.
...
- создавать `session_id` лениво, если сессия ещё не существует у RAIN;
...
`session_created=true` означает, что сессия была создана этим запросом или исходным запросом с тем же `Idempotency-Key`. Если `session_id` уже существовал у RAIN до создания run, RAIN возвращает `session_created=false`.
...
4. Ленивая сессия: RAIN создаёт `session_id` при первом `POST /chat/runs`, отдельный `PUT /chat/sessions/{session_id}` не нужен.
5. Обязательный `Idempotency-Key` в `POST /chat/runs` с возвратом того же `run_id` при повторе и `409 idempotency_conflict` при конфликте payload.
6. Структурированный `artifacts[]` с `bt_page.url`.
7. `GET /chat/sessions/{session_id}/messages` с `limit`, cursor-пагинацией и возможностью догрузки больших сообщений через `GET /chat/messages/{message_id}`.
8. Фиксация RAIN как владельца истории: АС КОДА может проксировать, краткоживуще кэшировать страницы или хранить локальную копию для UI/аудита, но не должна становиться единственным storage для агентского контекста.
9. Контрольные поля истории: `message_count`, `last_message_id`, `last_message_seq`, `history_version`, `sync_cursor`.
