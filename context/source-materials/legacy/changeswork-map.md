...
- Нормализованные feature-контейнеры созданы в `features/approvals`, `features/artifacts`, `features/deployments`, `features/packages`, `features/pilots`, `features/roles`, `features/scorecards`.
...
| `planning/mvp/current/tasks/legacy/` | старые подробные story/task breakdown, включая simulations | `context/source-materials/legacy/changeswork-full/planning/mvp/current/tasks/legacy/` |
...
| Simulations | baseline признаёт `Simulation` как existing coverage | отдельной legacy feature `simulations` в harness нет; деталка и список симуляций пока зафиксированы в baseline summary и raw snapshot |
