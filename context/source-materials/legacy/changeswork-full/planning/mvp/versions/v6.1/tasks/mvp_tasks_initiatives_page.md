# Задачи для страницы "Инициативы" (Initiatives)

## Обзор страницы

**Страница:** Список инициатив (Initiatives List View)
**Путь в макете:** Боковое меню → "Инициативы"
...
- Пагинация
- Кнопка "Создать инициативу" (только для ПРМ своего продукта)
- Клик на строку → переход к детальному просмотру

**Права доступа:**
- ПРМ видит только инициативы своего продукта
- Методолог видит все инициативы и может создавать
- Админ видит все инициативы

---

## AN-D1: Аналитика страницы "Инициативы"
...
### Summary
Детализация требований и проектирование UI/UX для страницы списка инициатив

### Description

Провести аналитику и детализацию требований для страницы "Инициативы":
...
2. **Спецификация UI/UX:**
   - Wireframe списка инициатив (таблица)
...
3. **Спецификация API контракта:**
   - GET /api/initiatives/ - список с фильтрацией
...
- API контракт для GET /api/initiatives/
- Wireframe или описание UI

---

## BE-D1: Backend для страницы "Инициативы"
...
### Summary
Реализация API endpoint для списка инициатив с фильтрацией, пагинацией и правами доступа

### Description

Реализовать Backend API для страницы списка инициатив:

1. **Миграция БД (если требуется):**
   - Проверить наличие всех необходимых полей в таблице `initiative`
   ...
   - **КРИТИЧНО:** Миграция данных - раскидать все существующие пилоты и симуляции по внедрениям
     - В новой архитектуре пилоты и симуляции ОБЯЗАТЕЛЬНО должны принадлежать внедрению (initiative_id NOT NULL)
     - Необходимо вручную или через скрипт миграции назначить initiative_id для всех существующих записей
     - Варианты:
     ...
     - После миграции данных добавить constraint NOT NULL на поля simulation.initiative_id и pilot.initiative_id

2. **API Endpoint: GET /api/initiatives/**
   ...
   **Формат ответа:**
   ...
   {
     "count": 42,
     "next": "http://api/initiatives/?page=2",
     "previous": null,
     "results": [
       {
         "id": "uuid",
...
- [ ] **Constraint NOT NULL добавлен на simulation.initiative_id и pilot.initiative_id**
- [ ] Индексы созданы для оптимизации
- [ ] GET /api/initiatives/ endpoint реализован
...
- BE-1 (таблица initiative должна существовать)
...
**Пример запроса:**
```bash
GET /api/initiatives/?product_id=credit_cards&status=active&search=премиум&page=1&page_size=20&ordering=-created_at
```
...
## FE-D1: Frontend для страницы "Инициативы"
...
### Summary
Реализация UI страницы списка инициатив с таблицей, фильтрами, поиском и пагинацией

### Description

Реализовать Frontend для страницы списка инициатив:

1. **Компонент InitiativesView:**
   - Табличное представление (Material-UI Table)
   - Столбцы: ID (INIT-XXX), Название, Продукт, Статус, Автор, Дата создания
...
9. **API интеграция:**
    - InitiativeService.getList(filters, page, pageSize, ordering)
    - Обработка ошибок (403, 404, 500)
    - Toast уведомления при ошибках

### Acceptance Criteria

- [ ] InitiativesView компонент реализован
...
function InitiativesView({ data, productFilter, onSelectInitiative }: InitiativesViewProps) {
...
6. **Права доступа:** Методолог МОЖЕТ создавать инициативы

---

## BE-D-LC: Lifecycle управление для Initiative
...
Реализовать lifecycle управление для Initiative:

1. **Автоматический расчет статуса:**
   - draft: пустая инициатива ИЛИ все вложенные сущности в draft
   - active: хотя бы одна вложенная сущность не в draft
   - deployed: хотя бы одно Внедрение в deployed
   - archived: переведено вручную (нельзя, если есть deployed Внедрения)

2. **Логика расчета статуса:**
   - При создании/обновлении Simulation → пересчет статуса Initiative
   - При создании/обновлении Pilot → пересчет статуса Initiative
   - При создании/обновлении Deployment → пересчет статуса Initiative
   - Кэширование статуса в поле initiative.status

3. **Правила архивирования:**
   - Нельзя архивировать Initiative с deployed Внедрениями
...
4. **API endpoints:**
   ...
   - GET /api/initiatives/{id}/status/ - получение статуса с причиной
...
5. **Валидация переходов:**
   ...
   - active → deployed: автоматически при deployed Initiative
...
- [ ] Правило deployed: хотя бы одно Initiative в deployed работает
- [ ] POST /api/initiatives/{id}/archive/ реализован
- [ ] Валидация: нельзя архивировать с deployed Changes
- [ ] При архивировании все вложенные сущности архивируются
- [ ] GET /api/initiatives/{id}/status/ возвращает статус с причиной
...
### Dependencies
- BE-D1 (базовый CRUD для Initiative)
- BE-S1 (API симуляций)
- BE-P1 (API пилотов)
- BE-C1 (API изменений)
...
class Initiative(models.Model):
    ...
    def calculate_status(self):
        ...
        # Если хотя бы одно Initiative в deployed
...
```python
@api_view(['POST'])
def archive_initiative(request, initiative_id):
    initiative = get_object_or_404(Initiative, id=initiative_id)
    
    # Проверка прав
    if not (is_prm(request.user, initiative.product) or is_admin(request.user)):
        return Response({'error': 'Недостаточно прав'}, status=403)
    
    # Проверка: нельзя архивировать с deployed Changes
    if initiative.changes.filter(status='deployed').exists():
        return Response({
