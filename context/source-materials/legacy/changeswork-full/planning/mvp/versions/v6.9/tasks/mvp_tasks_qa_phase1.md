...
5. **Цепочки (Lineage)**
- Страница "Цепочки" (`/chains`) + lineage widget.
- Автосоздание источников lineage при создании Pilot/Deployment на основе скоркарт.
- Preview lineage в формах (если предусмотрено задачами).
...
- Lineage: sources auto-link → Chains list → lineage detail/widget.
- Scorecards: статус отображается в списке/деталке; источники/использование отображаются.
...
- [ ] Работают страницы Скоркарт (list/detail/form) + интеграция статуса (`BE_SC_INT1`)
- [ ] Работают Цепочки (list + lineage widget + auto-link sources)
...
- Все задачи разработки MVP из Gantt v6.9 (backend + frontend), в частности:
  - `BE_AP1`, `BE_INT1`, `BE_PL1`, `BE_NOT1`
  - `BE_CL1`, `BE_C1`, `BE_CHD1`, `BE_CHF1`
  - `BE_SC_INT1`, `BE_SC1`, `BE_SCD1`, `BE_SCF1`
  - `BE_D1`, `BE_DD1`, `BE_CD1`
...
В `Gantt v6.9` QA стартует после завершения ключевых UI-веток (минимум `FE_NAV1` и `FE_SCF1`) и покрывает весь MVP.
