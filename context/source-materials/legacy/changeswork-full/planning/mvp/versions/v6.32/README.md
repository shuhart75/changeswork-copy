# MVP Planning v6.32 Commander Baseline (source: ProjectLibre Q2.pod)
...
- Выполнен полный проход по всем ресурсам actualized-слоя: обнаружены скрытые пересечения forecast-плана у `F1`, `Q1` и `TBD_FE` (из-за отпусков/праздников и длительностей в рабочих днях), после чего их нулевые задачи сдвинуты вправо.
- В `gantt/mvp_gantt_chart_current_actualized.puml` добавлена вертикальная отметка snapshot day `2026/04/17`.
