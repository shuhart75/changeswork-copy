...
3. **Спецификация API контракта:**
   - GET /api/ratification-queue/ - список Pilot/Deployment в `awaiting_ratification` (группировка по Initiative опционально)
     ...
     "results": [
       {
         "initiative": {
           "id": "uuid",
           "display_id": "INIT-001",
           "name": "Инициатива: оптимизация риск-стратегии Premium карт"
         },
         "items": [
           {
             "id": "uuid",
...
5. **Бизнес-логика Package:**
   ...
   **БП-1: Формирование Package**
   ...
   - Элементы могут быть из разных Initiatives (это допустимо по spec)
   - В MVP v6.1 считаем минимально обязательным выбор одного `ratifier` на Package
