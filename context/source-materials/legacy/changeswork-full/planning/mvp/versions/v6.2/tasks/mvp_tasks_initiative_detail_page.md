# Задачи для детальной страницы "Инициатива" (Initiative Detail)

## Обзор страницы

**Страница:** Детальный просмотр инициативы (Initiative Detail View)
**Путь в макете:** Клик на строку в списке инициатив → детальный просмотр
**Статус:** Абсолютно новая страница

**Примечание (v6.1/Gantt v6):** отдельная аналитика `AN-DD1` не выделена в диаграмме Ганта; считать её включенной в `AN-D1` (аналитика инициатив) или вынести в план отдельной задачей при необходимости.
...
- Кнопки действий:
  ...
  - "Архивировать" (для всех кроме draft, недоступно если есть deployed внедрения)
- Метаданные:
  - Продукт
  - Автор
  - Дата создания
...
1. **Вкладка "Симуляции":**
   ...
   - Список симуляций с карточками:
     ...
     - Связи с пилотами и внедрениями (чипы)
   - Кнопки для выбранных симуляций:
     - "Создать пилот из выбранных"
     - "Создать внедрение из выбранных"
...
2. **Вкладка "Пилоты":**
   ...
   - Список пилотов с карточками:
     ...
     - Связи с симуляциями и внедрениями (чипы)
   - Кнопка для выбранных пилотов:
     - "Создать внедрение из выбранных"
   - Inline форма создания пилота

3. **Вкладка "Внедрения":**
   - Кнопка "Новое внедрение"
   - Список внедрений с карточками:
     - ID (DEP-XXX), название (кликабельное)
     - Статус, создал, создано, развернуто
     - Количество скоркарт
     - Связанные скоркарты (блок с чипами)
     - Связи с симуляциями и пилотами (чипы)
   - Inline форма создания внедрения
...
4. **Вкладка "Скоркарты":**
   - Список скоркарт продукта с карточками:
     ...
     - Использование в пилотах и внедрениях (чипы)
...
- Удаление/архивирование инициативы

**Права доступа:**
- ПРМ может редактировать/удалять/архивировать только свои инициативы
- Методолог может просматривать все, но не может редактировать инициативы
- Админ может всё

---

## AN-DD1: Аналитика детальной страницы "Инициатива"
...
### Summary
Детализация требований и проектирование UI/UX для детальной страницы инициативы с вкладками для связанных сущностей (без вкладки "Обзор")

### Description

Провести аналитику и детализацию требований для детальной страницы инициативы:

1. **Детализация бизнес-требований:**
   - Уточнить правила отображения статуса (автоматический расчет)
   - Определить правила удаления инициативы (только draft)
   - Определить правила архивирования (нельзя если есть deployed внедрения)
...
3. **Спецификация API контрактов:**
   - GET /api/initiatives/{id}/ - детальная информация
   - GET /api/initiatives/{id}/simulations/ - список симуляций
   - GET /api/initiatives/{id}/pilots/ - список пилотов
   - GET /api/initiatives/{id}/changes/ - список изменений
   - GET /api/initiatives/{id}/scorecards/ - список скоркарт продукта
   - PUT /api/initiatives/{id}/ - редактирование
   - DELETE /api/initiatives/{id}/ - удаление
   - POST /api/initiatives/{id}/archive/ - архивирование
...
### Dependencies
- AN-D1 (требуется понимание базовой структуры Initiative)
...
## BE-DD1: Backend для детальной страницы "Инициатива"
...
1. **API Endpoint: GET /api/initiatives/{id}/**
...
2. **API Endpoint: GET /api/initiatives/{id}/simulations/**
...
3. **API Endpoint: GET /api/initiatives/{id}/pilots/**
...
4. **API Endpoint: GET /api/initiatives/{id}/changes/**
...
5. **API Endpoint: GET /api/initiatives/{id}/scorecards/**
...
6. **API Endpoint: PUT /api/initiatives/{id}/**
   ...
   **Формат ответа:** Обновленный объект Initiative

7. **API Endpoint: DELETE /api/initiatives/{id}/**

   **Валидация:**
   - Только для status = draft
   - HTTP 409 если status != draft

8. **API Endpoint: POST /api/initiatives/{id}/archive/**

   **Валидация:**
   - Нельзя архивировать если есть deployed изменения
   - HTTP 409 если есть deployed изменения
   - Устанавливает status = archived для Initiative и всех вложенных сущностей

9. **Бизнес-логика:**
   - Автоматический расчет статуса Initiative:
   ...
   - Фильтрация связанных сущностей по initiative_id
   - Права доступа:
...
- [ ] GET /api/initiatives/{id}/ endpoint реализован
- [ ] GET /api/initiatives/{id}/simulations/ endpoint реализован
- [ ] GET /api/initiatives/{id}/pilots/ endpoint реализован
- [ ] GET /api/initiatives/{id}/changes/ endpoint реализован
- [ ] GET /api/initiatives/{id}/scorecards/ endpoint реализован
- [ ] PUT /api/initiatives/{id}/ endpoint реализован
- [ ] DELETE /api/initiatives/{id}/ endpoint реализован
- [ ] POST /api/initiatives/{id}/archive/ endpoint реализован
...
- BE-D1 (базовый endpoint для Initiative)
...
def archive(self, request, pk=None):
    ...
    if initiative.changes.filter(status='deployed', rollback_at__isnull=True).exists():
        return Response(
    ...
    initiative.simulations.update(status='archived')
    initiative.pilots.update(status='archived')
    initiative.changes.update(status='archived')
...
## FE-DD1: Frontend для детальной страницы "Инициатива"
...
1. **Компонент InitiativeDetailView:**
...
9. **Навигация:**
   ...
   - Breadcrumbs: "Инициативы" → "INIT-XXX"
...
11. **API интеграция:**
    - InitiativeService.getDetail(id)
    - InitiativeService.getSimulations(id)
    - InitiativeService.getPilots(id)
    - InitiativeService.getChanges(id)
    - InitiativeService.getScorecards(id)
    - InitiativeService.update(id, data)
    - InitiativeService.delete(id)
    - InitiativeService.archive(id)
    - Обработка ошибок (403, 404, 409, 500)

### Acceptance Criteria

- [ ] InitiativeDetailView компонент реализован
...
- FE-D1 (требуется базовый InitiativesView)
...
function InitiativeDetailView({ initiativeId, onBack }: InitiativeDetailViewProps) {
  ...
  const handleArchive = async () => {
    ...
    if (confirmed) {
      try {
        await InitiativeService.archive(initiativeId);
