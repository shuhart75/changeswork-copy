...
**4 вкладки:**
1. **Требуют утверждения** - Pilot и Change со статусом 'requires_ratification'
2. **На согласовании/утверждении** - Pilot и Change со статусом 'in_approval' или 'in_ratification'
3. **Отклонено** - Pilot и Change со статусом 'rejected'
4. **История согласований/утверждений** - лог всех согласований

**Функционал:**
- Группировка Pilot и Deployment по Initiative
...
**Особенности:**
- Package группирует Цепочки с одинаковым маршрутом согласования
- Критичность Package = MAX(chain.priority) среди всех Цепочек
...
1. **Детализация бизнес-требований:**
   - Определить правила группировки Pilot и Change по Deployment
   - Уточнить логику формирования Package (одинаковые маршруты)
...
3. **Спецификация API контракта:**
   - GET /api/packages/ - список с группировкой
   - POST /api/packages/submit_for_ratification/ - batch отправка на утверждение
   - POST /api/packages/recall/ - отзыв с согласования
   - GET /api/packages/history/ - история согласований
   - Параметры запроса (product_id, status, page, page_size)
   - Формат ответа (группировка по Initiative)
   - Коды ошибок (403, 404, 500)

4. **Определение полей для отображения:**
   - Группировка: Initiative (название)
...
1. **API Endpoint: GET /api/packages/**
   ...
   **Параметры запроса:**
   ...
   - `status` (optional) - фильтр по статусу (requires_ratification/in_approval/in_ratification/rejected)
...
2. **API Endpoint: POST /api/packages/submit_for_ratification/**
   ...
   **Бизнес-логика:**
   - Валидация: все элементы должны иметь статус 'requires_ratification'
   - Создание Package (если элементы из одного Deployment)
   - Создание ApprovalWorkflow для Package
   - Назначение ratifier для каждого элемента
   - Отправка email уведомлений ratifier
   - Обновление статусов элементов → 'in_ratification'
...
5. **Бизнес-логика Package:**
   ...
   **БП-1: Формирование Package**
   - Package группирует Pilot и Change из одного Deployment
   - Все элементы должны иметь одинаковый маршрут утверждения (ratifier)
   - Критичность Package = MAX(criticality) всех элементов

   **БП-2: Batch отправка на утверждение**
   - Валидация: все элементы в статусе 'requires_ratification'
   - Валидация: все элементы из одного Deployment
   - Создание Package
   - Создание ApprovalWorkflow для Package
   - Назначение ratifier для всех элементов
...
- [ ] Группировка по Initiative работает
...
1. **Компонент PackagesView:**
   - 4 вкладки: Требуют утверждения, На согласовании/утверждении, Отклонено, История
   - Группировка элементов по Initiative
   - Responsive дизайн

2. **Вкладка "Требуют утверждения":**
   - Отображение Pilot и Deployment со статусом 'requires_ratification'
   - Группировка по Initiative (название инициативы)
...
3. **Вкладка "На согласовании/утверждении":**
   - Отображение Pilot и Deployment со статусом 'in_approval' или 'in_ratification'
...
4. **Вкладка "Отклонено":**
   - Отображение Pilot и Deployment со статусом 'rejected'
   - Группировка по Initiative
...
8. **Группировка по Initiative:**
   - Заголовок группы: "Инициатива: {название}"
...
3. **Группировка по Initiative** - визуальная группировка элементов
