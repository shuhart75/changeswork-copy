# Задачи: адаптация существующего ЖЦ Пилота под согласование/утверждение (Pilot Lifecycle, MVP)

## Обзор

**Компонент:** Жизненный цикл Пилота (Pilot Lifecycle)
**Статус:** Адаптация существующего ЖЦ пилота под согласование/утверждение (MVP)

### Что реализуется:

> Граница ответственности (важно): `BE-PL1` описывает/реализует **модель версий PilotVersion, статусы и action endpoints** жизненного цикла пилота.  
> Автоматические переводы статусов на событиях согласования/утверждения (ApprovalInstance approved/rejected/cancelled, awaiting_ratification) выполняются в `BE-INT1`.
...
- При публикации новой версии предыдущая active-версия переводится в `inactive` (поведение уже поддержано в текущем ЖЦ пилота; в рамках MVP важно сохранить/не сломать).
...
### Summary
Адаптация существующего жизненного цикла пилота к процессу согласования/утверждения (Approval/Ratification): перевод согласования на уровень `PilotVersion`, поддержка rejected-статусов, action endpoints и версионирование без изменения базовой логики "текущая версия / не-active".

### Description

Адаптировать существующий жизненный цикл Пилота под согласование/утверждение:

1. **Модель данных Pilot + PilotVersion (адаптация/расширение, без переписывания ЖЦ):**

   В коде/БД уже существует ЖЦ пилота и поведение "старая версия становится не-active". В рамках MVP требуется:
   - убедиться, что статусы `requires_activation`, `approval_rejected`, `ratification_rejected` присутствуют на уровне **версии** (`PilotVersion`)
   - если каких-то полей/статусов нет: добавить их (миграции), не ломая существующие статусы (`active/completed/archived`) и текущую логику `inactive/not-active`.
   - сохранить `Pilot.current_version` (или аналог) как указатель на текущую активную версию.

2. **API Endpoint: POST /api/pilot-versions/{id}/submit_for_approval/** (версия пилота, не "пилот" целиком)
...
3. **API Endpoint: POST /api/pilot-versions/{id}/complete/** (завершение текущей версии/пилота)
