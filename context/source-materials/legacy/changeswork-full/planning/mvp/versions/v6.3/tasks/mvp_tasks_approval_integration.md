# Задачи для интеграции ЖЦ Simulation/Pilot/Deployment с процессом согласования (MVP v6.3)

**Источник правды:** [`spec/domain_model.md`](/home/reutov/Documents/AI/changesWork/spec/domain_model.md)

## Обзор

**Компонент:** Связка `ApprovalInstance` (решения) с жизненными циклами `Simulation`, `Pilot` и `Deployment`.  
**Статус:** Новая разработка

### Что реализуется

- Simulation:
  - `completed -> requires_approval` при отправке на согласование (submit)
...
2. **Матрица переходов (минимально для MVP v6.3)**
   - Simulation:
     - approved/ratified -> `approved`
     - rejected -> `rejected`
     - cancelled -> возврат в `completed` (если требуется сохранить расчет) или `draft` (если требуется пересчет)
   - Pilot:
...
- [ ] Внедрены обработчики событий `ApprovalInstance` и переходы статусов Simulation/Pilot/Deployment
...
- `BE-AP1` ([mvp_tasks_approval_process_backend.md](/home/reutov/Documents/AI/changesWork/planning/mvp/versions/v6.3/tasks/mvp_tasks_approval_process_backend.md))
- `BE-SL1` ([mvp_tasks_simulation_lifecycle.md](/home/reutov/Documents/AI/changesWork/planning/mvp/versions/v6.3/tasks/mvp_tasks_simulation_lifecycle.md))
