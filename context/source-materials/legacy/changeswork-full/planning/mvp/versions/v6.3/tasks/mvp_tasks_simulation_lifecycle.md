...
- requires_approval - требует согласования (отправлена на согласование)
- approved - согласована/утверждена (результаты можно использовать)
- rejected - отклонена (требует корректировок/перезапуска)

**Переходы:**
- draft → running (запуск симуляции)
- running → completed (успешное завершение)
- running → failed (завершение с ошибкой)
- completed → requires_approval (отправка на согласование)
- requires_approval → approved (согласование/утверждение успешно)
- requires_approval → rejected (отклонение)
- rejected → draft (корректировка параметров и повторный запуск)

**Бизнес-правила:**
- Только ПРМ (свой продукт) или Админ могут управлять переходами
- Симуляция участвует в процессе согласования через `ApprovalInstance` (см. `BE-AP1` + `BE-INT1`)
...
5. **Бизнес-правила:**
   ...
   **БП-4: Интеграция с согласованием**
   - Отправка на согласование создаёт `ApprovalInstance(target_type='simulation')` (см. `BE-AP1`)
   - Обработчики `BE-INT1` синхронизируют статус симуляции:
...
### Dependencies
`BE-AP1` + `BE-INT1` (процесс согласования/интеграция)
...
3. **Участие в согласовании** - через `ApprovalInstance` + интеграцию `BE-INT1`
