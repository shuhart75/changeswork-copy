...
**Статус:** Новая разработка (в рамках v6.3)

### Что реализуется

**Объекты согласования (target):**
- `Simulation` (когда status = `completed` или `rejected` для повторной отправки)
...
### Summary
Реализация `ApprovalInstance` + этапов/назначений/решений и API для отправки/принятия решений по Simulation/Pilot/Deployment/Package.

### Scope (минимально необходимое для MVP v6.3)

1. **Модель данных (Django/DRF, названия как в spec):**
   - `ApprovalInstance`:
     - `target_type` (`simulation`/`pilot`/`deployment`/`package`)
     - `target_id` (UUID)
...
2. **API: отправка на согласование**
   - `POST /api/simulations/{id}/submit_for_approval/`
   ...
   - Валидации:
     - target в допустимом статусе (Simulation: `completed` или `rejected`; Pilot: `requires_activation` или `requires_correction`; Deployment: `requires_approval`)
   ...
   - Побочные эффекты:
     ...
     - переводим Simulation: `completed -> requires_approval` (или остаётся `rejected`, если переотправка с тем же объектом)
     - переводим Pilot: `draft -> requires_activation` (или сохраняем `requires_correction`, если переотправка)
     - переводим Deployment: `draft -> requires_approval` (если в MVP допускается черновик)
...
3. **API: принятие решений**
   ...
   - Логика:
     ...
     - если в этапе есть reject: этап/инстанс -> `rejected`, возвращаем target в рабочий статус:
       - Simulation -> `rejected`
       - Pilot -> `requires_correction`
       - Deployment -> `draft` (если черновик поддерживается) или `requires_approval` (если редактирование возможно без возврата в draft)
...
- [ ] `POST /api/simulations/{id}/submit_for_approval/` реализован и валидирует роли/самого себя
...
- `BE-SL1` ([mvp_tasks_simulation_lifecycle.md](/home/reutov/Documents/AI/changesWork/planning/mvp/versions/v6.3/tasks/mvp_tasks_simulation_lifecycle.md)) для статусов Simulation
- `BE-CL1` ([mvp_tasks_deployment_lifecycle.md](/home/reutov/Documents/AI/changesWork/planning/mvp/versions/v6.3/tasks/mvp_tasks_deployment_lifecycle.md)) для статусов Deployment
- `BE-PL1` ([mvp_tasks_pilot_lifecycle.md](/home/reutov/Documents/AI/changesWork/planning/mvp/versions/v6.3/tasks/mvp_tasks_pilot_lifecycle.md)) для статусов Pilot
- `BE-NOT1` ([mvp_tasks_notifications.md](/home/reutov/Documents/AI/changesWork/planning/mvp/versions/v6.3/tasks/mvp_tasks_notifications.md)) для email/уведомлений
