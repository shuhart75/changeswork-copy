# MVP Planning v6.36 Commander Baseline (source: ProjectLibre Q2.pod)
...
- Gantt tracking / weekly single-page experiment: `gantt/mvp_gantt_chart_current_actualized_weekly.puml`
- Gantt tracking / compact single-page experiment: `gantt/mvp_gantt_chart_current_actualized_compact.puml`
- Gantt tracking / aggressive single-page experiment: `gantt/mvp_gantt_chart_current_actualized_aggressive.puml`
...
- В начало actualized PlantUML-диаграммы добавлен динамический today-marker: подсветка текущего дня (`today is colored in LightSalmon`) и milestone `[Мы сейчас здесь]` на `%now()`.
- На `2026-05-21` добавлен milestone релиза `RSCON-2438 КОДА 01.024.00`; в рамках этого релиза планируется выкатить весь блок скоркарт.
- Для попытки уместить диаграмму на одну страницу добавлены два альтернативных view-файла: weekly-view через `printscale weekly` и compact-view через уменьшение zoom с `1.5` до `1`.
- Добавлен третий, более агрессивный single-page view: weekly-view с глобальным `scale 2/3`, чтобы сильнее ужать вывод при рендере.
