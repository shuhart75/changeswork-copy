...
- Added two alternative PlantUML views for fitting the actualized gantt onto one page: weekly scale (`mvp_gantt_chart_current_actualized_weekly.puml`) and compact daily scale (`mvp_gantt_chart_current_actualized_compact.puml`).
- Added a third, more aggressive PlantUML single-page view: `mvp_gantt_chart_current_actualized_aggressive.puml`, based on weekly scale plus global `scale 2/3`.
