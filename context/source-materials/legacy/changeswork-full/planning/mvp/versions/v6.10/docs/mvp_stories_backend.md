...
- [ ] Таблица `package_item` создана с полями: id, package_id, item_type (enum: pilot/change), item_id, status (enum: pending/ratified/ratification_rejected/recalled/released), created_at
...
- Права доступа: ПРМ читает все продукты (read-only для чужих), но изменяет только свой продукт; Админ видит/изменяет все
...
- [ ] GET /api/deployments/ - список Deployment
  ...
  - Права: ПРМ читает все продукты (read-only для чужих), Админ видит все
...
- [ ] PATCH /api/deployments/{id}/ - обновление
  ...
  - Права: ПРМ только для своего продукта, Админ
...
- [ ] POST /api/packages/ - создание Package и отправка на утверждение
  ...
  - Валидация MVP: минимум 2 items (минимальный размер пакета = 2)
  - Валидация MVP: у Package нет привязки к продукту; допускается микс элементов из разных продуктов/инициатив (ограничения только правами на доступ к самим элементам)
  - Создаёт Package, PackageItem (status: pending) и ApprovalInstance одновременно
  - priority вычисляется на лету (MAX от items)
  - Права: ПРМ только если все items принадлежат его продукту; Методолог/Админ без ограничения по продуктам (при наличии доступа)
- [ ] GET /api/packages/ - список с фильтрацией
  - Фильтры: priority (и, при необходимости, фильтрация на уровне прав/доступа к элементам)
  - Сортировка: priority, created_at
  - Пагинация: 20 элементов
  - Права: ПРМ читает все (read-only), Админ видит все
...
- [ ] POST /api/approval-instances/{id}/reject/ - отклонение
  ...
  - Target получает статус `approval_rejected` (reject на Approval) или `ratification_rejected` (reject на Ratification), не возвращается в draft
...
2. Отклонение на первом этапе: submit → один отклонил на approval → target = `approval_rejected`
3. Отклонение на втором этапе: submit → все одобрили на approval → один отклонил на ratification → target = `ratification_rejected`
...
- [ ] Тест: Права доступа ПРМ
  ...
  - ПРМ видит все Deployment (read-only), но может изменять только свои
