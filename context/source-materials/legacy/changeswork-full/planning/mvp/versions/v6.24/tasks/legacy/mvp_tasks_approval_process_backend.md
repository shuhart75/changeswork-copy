...
**Статус:** Новая разработка (в рамках v6.7)
...
- **Уточнение MVP v6.7:** если `auto_ratification=false`, элемент попадает в очередь `awaiting_ratification` и дальше по команде пользователя может быть отправлен на Ratification:
...
### Summary
Зафиксировать MVP-правила процесса согласования (Approval) и утверждения (Ratification) и подготовить единый контракт (статусы, очереди, recall/reject, Package, права, API), включая UX-спецификацию выбора маршрута согласования при отправке сущности на согласование.
...
2. **UX-спецификация выбора маршрута согласования/утверждения (MVP):**
   - Диалог (условное название) `ApprovalRouteDialog`, который открывается по действию:
     - "Отправить на согласование" для `PilotVersion`
     - "Отправить на согласование" для `DeploymentVersion`
   - Что настраивается пользователем:
     - 0..* этапов Approval (этапы последовательно, внутри этапа параллельно по назначенным approver-ам)
     - для каждого назначенного approver: тип назначения `required` (обязательно) или `inform` (ознакомление), если поддерживаем в MVP
     - флаг `ratification_required` (по умолчанию `false`)
     - флаг `auto_ratification` (имеет смысл только при `ratification_required=true`)
     - выбор `ratifier` (обязателен только если `ratification_required=true` и `auto_ratification=true`)
   - Валидации в UI (дублируются на BE):
     - нельзя выбрать самого себя как approver/ratifier
     - если `ratification_required=false`: должен быть выбран хотя бы один approver (минимум один этап и минимум один человек)
     - если `ratification_required=true`: можно отправить без approvers (только ratifier), но `ratifier` должен быть выбран при `auto_ratification=true`
   - Выходной контракт диалога: формирует payload `approval_stages[]`, `ratification_required`, `auto_ratification`, `ratifier_id`
   - Примечание: диалог выбора маршрута **ratification** для формирования `Package` остаётся в зоне `AN_PKG1`, но модели данных и payload должны быть единообразны.

3. **API-спецификация MVP (черновик контракта):**
...
- [ ] UX-спецификация `ApprovalRouteDialog` согласована: поля, валидации, выходной payload, где вызывается (PilotVersion/DeploymentVersion submit)
...
### Scope (минимально необходимое для MVP v6.7)
...
### Dependencies
   ...
   - `BE-CL1` ([mvp_tasks_deployment_lifecycle.md](/home/reutov/Documents/AI/changesWork/planning/mvp/versions/v6.7/tasks/mvp_tasks_deployment_lifecycle.md)) для статусов Deployment
   - `BE-PL1` ([mvp_tasks_pilot_lifecycle.md](/home/reutov/Documents/AI/changesWork/planning/mvp/versions/v6.7/tasks/mvp_tasks_pilot_lifecycle.md)) для статусов Pilot
   - `BE-NOT1` ([mvp_tasks_notifications.md](/home/reutov/Documents/AI/changesWork/planning/mvp/versions/v6.7/tasks/mvp_tasks_notifications.md)) для email/уведомлений
