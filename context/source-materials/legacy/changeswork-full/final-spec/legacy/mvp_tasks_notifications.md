...
- Отправка на согласование (Pilot/Deployment/Package)
   ...
   class Notification(models.Model):
       ...
       notification_type = models.CharField(
           ...
           choices=[
               ...
               ('deployment_deployed', 'Внедрение внедрено'),
               ('deployment_rolled_back', 'Внедрение откачено')
           ]
       )

       # Связь с сущностью
       entity_type = models.CharField(max_length=50)  # 'pilot', 'deployment', 'package'
...
2. **Email шаблоны:**
   ...
   **Шаблон для отправки на согласование (approval_request.html):**
   - Приветствие
   - Текст: "Направляем на согласование элементы риск-стратегий:"
...
4. **Интеграция с событиями:**
   ...
   **Signals для автоматической отправки:**
   ...
   def handle_approval_instance_events(sender, instance, created, **kwargs):
       """Обработка событий процесса согласования (ApprovalInstance)"""
       if created:
           # Отправка на согласование: уведомляем назначенных на первый этап
           stage = instance.stages.order_by('stage_order').first()
           approvers = [a.user for a in stage.assignments.all()]
           NotificationService.send_approval_request(instance, approvers)

       elif instance.status == 'approved':
           # Процесс завершен успешно
           NotificationService.send_approval_approved(instance, instance.submitted_by)

       elif instance.status == 'rejected':
           # Отклонено
           reason = get_rejection_reason(instance)
           NotificationService.send_approval_rejected(instance, instance.submitted_by, reason)
   ...
   def handle_stage_completion(sender, instance, **kwargs):
       ...
       if instance.status == 'approved':
           ...
           next_stage = instance.approval_instance.stages.filter(
               stage_order=instance.stage_order + 1
           ).first()

           if next_stage:
               # Уведомление согласующим следующего этапа
               approvers = [a.user for a in next_stage.assignments.all()]
